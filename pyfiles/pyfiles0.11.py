            ...
            try:
                ...
                # 💥 ПАПКА
                    ...
                    for root_dir, dirs, files in os.walk(path):
                        ...
                        # файлы
                        for fname in files:
                            full = os.path.join(root_dir, fname)  # ✅ сначала создаём
...
def show_whats_new():
    ...
    text.insert("1.0",
        ...
        "MAJOR UPDATES: \n"
        "\n"
        ">> Cache function is added: when you are cleaning in list smth what you dont want to move in some folder, instead of crash you get this file or folder in cache folder created in place where program exists\n"
        ">> ToolTip is added: Move your mouse on file and look on directory showed up \n"
        ">> File replacing function added: if moving/copying already exist file there it will ask for replacing or skipping\n"
        ">> Fatal error folder and/or file: now its not cleaning and showing in red color\n"
        ">> About... and What's new? new UI added\n"
        ">> Autoputting prototype in prefix date parameters: writing 22022002 automatically makes it 22.02.2002\n"
        ">> File without extension is showing special symbol: ?, if file haves unknown extension then just empty file\n"
        ">> UAC mode is always ON from now to prevent user with restrictions moves bugs\n"
        ">> Status bar logic updated\n"
        ">> Logging logic is updated\n"
        ">> Console logging logic is updated\n"
        "\n"
        "OTHER UPDATES & FIXES: \n"
        "\n"
        ">> Additional info after moving/copying is added\n"
        ">> Copying few directories function fixed, now its working for few files in drag mode\n"
        ">> Bug with directory shown in Source files fixed \n"
        ">> Copy mode function is fixed, all inside folder copying together with folder\n"
        ">> Random UI bug and function after swtiching modes Move/Copy is fixed\n"
        ">> After moving files list is cleaning\n"
        ">> Placeholder prefix date in Create mode showing is fixed\n"
        ">> Choosed date in Create mode now is showing in preview and working\n"
        ">> Delete mode list showing is fixed\n"
        ">> Adding same folder to copy/move is fixed\n"
...
duplicate_btn = tk.Button(left_panel, text="COPY", command=lambda: set_mode("COPY"), **btn_style)
...
unknown_icon = ImageTk.PhotoImage(
    Image.open(resource_path("unknown.png")).resize((16, 16))
)
