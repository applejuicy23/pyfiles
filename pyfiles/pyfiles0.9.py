...
def apply_theme(name):
    ...
    content_box.config(bg=entry_bg, fg=entry_fg, insertbackground=entry_fg)
    ...
    for f in (center_frame, create_frame, delete_frame, left_create, right_create, left_delete, right_delete):
        try:
            f.config(bg=t["bg"])
        except:
            pass
...
def select_tab(tab):
    tab.config(bg="white")
    root.after(150, lambda: tab.config(bg="SystemButtonFace"))
...
content_scroll_x = tk.Scrollbar(content_container, orient="horizontal", command=content_box.xview)
