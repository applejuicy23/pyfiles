...
def show_about():
    ...
    tk.Label(
        about_window,
        text="PyFiles v0.10.1\n\nNot-a-Simple GUI tool\nPython 3.14 + tkinter\nv0.10.1 - UNDER CONSTRUCTION",
        bg="#1e1e1e",
        fg="white",
        justify="center"
    ).pack(pady=10)
...
def show_whats_new():
    ...
    text.insert("1.0",
        ">> Console and LOG system in stable state \n"
        ">> Console&LOG logic updated \n"
        ">> Copy mode fixed\n"
        ">> Delete mode is under construction (Bin ability is broken)\n"
    )
...
def delete_files():
    ...
    set_operation_status("Files deleted", logs, errors)
...
def delete_from_bin():
    ...
    for item_id in selected:
        path = delete_bin_tree.item(item_id, "values")[0]
        name = os.path.basename(path)

        # 1. восстановить
        for item in items:
            try:
                if path.lower() in item.original_filename().lower():
                    item.undelete()
                    break
            except:
                pass
        ...
        try:
            if os.path.exists(path):
                ...
                log_to_console(f"Deleted permanently: {name}")

        except Exception as e:
            show_error(str(e))

        delete_bin_tree.delete(item_id)
...
def restore_file():
    ...
    for item_id in selected:
        ...
        for item in items:
            try:
                bin_name = os.path.basename(item.original_filename())
        ...
        if restored:
            log_to_console(f"Restored: {filename}")
        else:
            show_error(f"{filename} not found in recycle bin")

        # удаляем из UI и списка
        delete_bin_tree.delete(item_id)
...
def save_log(logs, errors, action="Process"):
    ...
    with open(log_filename, "w", encoding="utf-8") as f:
        f.write("LOG\n")
        f.write("Version: PyFiles v0.10.1\n")
...
def refresh_all():
    if current_delete_tab.get() == "list":
        search_files()
    else:
        load_recycle_bin()
