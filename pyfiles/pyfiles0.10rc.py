...
def show_about():
    ...
    tk.Label(
        about_window,
        text="PyFiles v0.10rc\n\nNot-a-Simple GUI tool\nPython 3.14 + tkinter\nv0.10rc - RELEASE CANDIDATE 2",
        bg="#1e1e1e",
        fg="white",
        justify="center"
    ).pack(pady=10)
...
def show_whats_new():
    ...
    text.insert("1.0",
        ">> Choosing Folder is working quite stable\n"
        ">> Create mode is updated\n"
        ">> Now you can customise your prefixes options \n"
        ">> Function DEEPSCAN and SAFEMODE added\n"
        ">> Now you can delete permanently files or with recovery\n"
        ">> Console and LOG system in preview mode \n"
        ">> Refresh button in Delete mode added for a while \n"
        ">> Prefix EXT is removed\n"
    )
...
def switch_delete_tab(tab):
    ...
    if tab == "list":
        ...
        delete_bin_tree.pack(in_=delete_container, fill="both", expand=True)
...
def delete_files():
    ...
    try:
        ...
        existing = [
            delete_bin_tree.item(i, "values")[0]
            for i in delete_bin_tree.get_children()
        ]
...
def show_create_preview():
    ...
    name = get_entry_value(format_entry)
    ext = get_entry_value(format_entry)
...
def show_bin_menu(event):
    ...
    menu.add_command(
        label="Restore",
        command=lambda: restore_file(path)
    )
...
def restore_file(path):
    ...
    for item in winshell.recycle_bin():
        try:
            ...
            original_name_only = os.path.splitext(original_name)[0].lower()

            if os.path.normcase(os.path.abspath(original)) == os.path.normcase(os.path.abspath(path)):
                item.undelete()
                set_success(f"Restored: {filename}")
                return

            if original_name_only == name_only:
                item.undelete()
                set_success(f"Restored (no ext match): {filename}")
                return
...
tk.Label(left_delete, text="Deep scan level:").pack(anchor="w")
