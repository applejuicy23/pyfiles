...
def adjust_tree_column(tree, name, path):
    ...
    path_width = font.measure(path) + 20

    tree.column("#0", width=max(tree.column("#0", "width"), name_width))
    tree.column("path", width=max(tree.column("path", "width"), path_width))
...
def show_whats_new():
    ...
    text.insert("1.0",
        ...
        ">> Working with Move/Copy mode is fixed\n"
        ">> Now you can dechoose chosen file \n"
        ">> Bug with Error color fixed\n"
        ">> Copy function is fixed\n"
        ">> Drag function is updated\n"
        ">> Folder logic is updated\n"
    )
...
root.bind("<Delete>", delete_file_key)
