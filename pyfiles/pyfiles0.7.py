...
def set_mode(mode):
    ...
    duplicate_btn.state(["!pressed"])
    create_btn.state(["!pressed"])
    delete_btn.state(["!pressed"])

    if mode == "MOVE":
        move_btn.state(["pressed"])
        main_frame.pack(pady=10)
        set_status("Mode: MOVE")

    elif mode == "DUPLICATE":
        duplicate_btn.state(["pressed"])
        main_frame.pack(pady=10)
        set_status("Mode: DUPLICATE")

    elif mode == "CREATE":
        create_btn.state(["pressed"])
        show_create()
        set_status("Mode: CREATE")
...
def delete_found_files():
    ...
    if not items:
        show_error("Select files to delete")
        return
...
move_btn = ttk.Button(top_frame, text="Move", width=12,
                      command=lambda: set_mode("MOVE"))
move_btn.grid(row=0, column=0)

duplicate_btn = ttk.Button(top_frame, text="Duplicate", width=12,
                           command=lambda: set_mode("DUPLICATE"))
...
create_btn = ttk.Button(
    ...
    text="Create",
    width=12,
    command=lambda: set_mode("CREATE")
)
create_btn.grid(row=0, column=2)

delete_btn = ttk.Button(top_frame, text="Delete", width=12,
                        command=lambda: set_mode("DELETE"))
delete_btn.grid(row=0, column=3, padx=(0,20))

about_btn = ttk.Button(top_frame, text="About", width=10, command=show_about)
about_btn.grid(row=0, column=4, padx=10)

whatsnew_btn = ttk.Button(top_frame, text="What's New", width=12, command=show_whats_new)
whatsnew_btn.grid(row=0, column=5)
...
file_name_entry.insert(0, "mertz")
...
prefix_entry.insert(0, "~1{file}cool~num")
...
choose_dest_btn.pack(side="left", padx=(0,40))
...
delete_dest_btn.pack(side="right", padx=(40,0))
